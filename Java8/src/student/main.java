...
public class main {
    public static void main(String[] args) {
        ...
        List<Student> lstStuName = list.stream().filter(data -> data.getFirstName().startsWith("A")).collect(Collectors.toList());
        ...
        List<String> lstDepartments = list.stream().map(Student::getDepartmantName).distinct().collect(Collectors.toList());
        ...
        Map.Entry<String, Long> stringLongEntry = list.stream().collect(Collectors.groupingBy(Student::getDepartmantName, Collectors.counting())).entrySet().stream()
        ...
        Set<Student> lstDelhistudent = list.stream().filter(dt -> dt.getCity().equals("Delhi"))
                               .sorted(Comparator.comparing(Student::getFirstName)).collect(Collectors.toSet());
